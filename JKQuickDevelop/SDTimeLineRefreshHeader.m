...
- (void)setupView
{
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Home_Icon_Cricle_Bright"]];
...
- (void)updateRefreshHeaderWithOffsetY:(CGFloat)y
{
    ...
    CGFloat topInsetY = self.scrollView.contentInset.top;
    if (@available(iOS 11.0, *)) {
        topInsetY            = self.scrollView.safeAreaInsets.top;
    }
    CGFloat rotateValue = y / 50.0 * M_PI;
    CGFloat dragDelta = y + topInsetY;
    if (y < -topInsetY) {
        y = -topInsetY;
        ...
        } else if (!self.scrollView.isDragging && self.refreshState == SDWXRefreshViewStateWillRefresh && dragDelta < -30) {
